<div class="relative">
    <form  wire:submit.prevent="update">
        <input type="text" name="name" placeholder="Workspace Title" class="input---field" wire:model.defer="name">
        @error('name')
        <small class="field-error-message">
            <span>{{$message}}</span>
        </small>
        @enderror
        <textarea name="description" rows="3" placeholder="Workspace Description..." class="input---field" wire:model.defer="description" ></textarea>
        @error('color')
        <small class="field-error-message">
            <span>{{$message}}</span>
        </small>
        ...
        {{-- <div class="input---field">
            <i class="fas fa-cloud-download-alt mr-2"></i>Cloud Storage
            <input type="checkbox" name="cloud_storage" checked class="form-checkbox text-green-550 border-green-550 bg-green-550 rounded-full focus:ring-0 float-right mt-1" />
        </div> --}}
        {{-- <input type="text" placeholder="Cloud URL {Folder Name}" class="input---field"> --}}
        <div class="input---field">
