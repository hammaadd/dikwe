...
@forelse($users as $user)
        ...
        <div class="w-full py-2 flex flex-wrap items-center justify-between rounded-xl">
            <div class="flex flex-wrap items-center">
                <div class="w-full flex items-center">
                    <a href="#" class="block relative">
                        ...
                        @if($user->follower->profile_img == null)
                            https://ui-avatars.com/api/?background=EAF7F0&name={{ str_replace(' ','+' ,$user->follower->name) }}
                        @else
                            {{asset('user_profile_images/'.$user->follower->profile_img)}}
                        @endif
            ...
            <div class="flex flex-wrap items-center">
                @if(in_array($user->follower->id,$addedUsers,true))
                    ...
                    wire:click="removeUser({{$user->follower->id}})">
                ...
                @else
                    <button class=" bg-green-550 text-white text-sm text-center rounded-full px-2 py-1 mx-3 lg:mx-0 xl:mx-3 hover:bg-green-150 hover:text-gray-600" title="Add user to this notes restricted."
                        wire:click="addUser({{$user->follower->id}})">
