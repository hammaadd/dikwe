<div class="relative mt-4">
   ...
   <div class="flex flex-wrap justify-between items-center px-4">
    <div class="mx-auto md:mx-0 mb-4">
        ...
        <span class="bg-green-550 font-bold py-2 px-3 mx-2 rounded-xl text-white focus:outline-none"><i class="fas fa-clipboard mr-2"></i><span class="hidden xl:inline-block">{{$noteHeading}}</span> <span class="counts md:ml-3">@if($notes) {{$count}} @endif</span></span>
    ...
    @forelse($notes as $note)
        <div class="w-full md:w-11/12 mx-auto rounded-lg shadow-md mb-4 md:mb-8">
            <div class="flex flex-col-reverse sm:flex-row justify-between relative pt-2 px-2 md:px-4">
                ...
                <div x-data="{ bShow: false }">
                    ...
                    @if($note->owner->id == Auth::id())
                        <li>
                            <a href="#" class="dropdown-item" wire:click="passNoteId({{$note->id}})" x-on:click="bShow = !bShow, $dispatch('shownoteedit') , @this.passNoteId({{$note->id}})">
                    ...
                    @if($note->owner->id == Auth::id())
                        <li>
                            <a href="#" class="dropdown-item" x-on:click="bShow = !bShow" wire:click="delete({{$note->id}})">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center relative bg-green-150 px-2 md:px-4 py-2">
                ...
                @if($note->owner->id == Auth::id())
                    <input type="checkbox" value="{{$note->id}}" name="select_note" id="note-{{$note->id}}" wire:model="select_note" class="form-checkbox text-green-550 border-green-550 focus:ring-0" @if($note_selected) checked @endif>
                @endif
                </div>
            </div>
        </div>
        ...
        <div class="p-3 rounded-xl shadow-md flex flex-wrap w-full">
            <div class="self-start w-full">
                <div class="flex flex-row justify-between relative" x-data="{ bShow: false }">
                    ...
                    <button x-on:click=" bShow = true " class="text-gray-400 bg-green-150 rounded-xl mx-1 px-2 h-10 w-10 hover:text-green-550 focus:outline-none border">
                        <i class="fas fa-ellipsis-v text-lg align-middle"></i>
                    </button>
                    <ul
                        x-show="bShow"
                    ...
                    @if($note->owner->id == Auth::id())
                        <li>
                            <a href="javascript:void(0)" wire:click="passNoteId({{$note->id}})" x-on:click="bShow = !bShow, $dispatch('shownoteedit') , @this.passNoteId({{$note->id}})" class="dropdown-item">
                                <i class="fas fa-edit dropdown-item-icon"></i>
                                <span class="ml-2">Edit Note</span>
                            </a>
                        </li>
                    ...
                    @if($note->owner->id == Auth::id())
                        <li>
                            {{-- <a href="javascript:void(0)" class="dropdown-item" wire:click="delete({{$note->id}})" x-on:click="bShow = !bShow, @this.delete({{$note->id}})"> --}}
                                <a href="javascript:void(0)" class="dropdown-item"  x-on:click="bShow = !bShow" wire:click="delete({{$note->id}})">
                                <i class="fas fa-trash-alt dropdown-item-icon"></i>
                                <span class="ml-2">Delete Note</span>
                            </a>
                        </li>
                    ...
                    <li title="On Click Export File will be downloaded.">
                        <a href="javascript:void(0)" wire:click="downloadFile({{$note->id}})" class="dropdown-item" x-on:click="bShow = !bShow">
                            <i class="fas fa-file-export dropdown-item-icon"></i>
                            <span class="ml-2">Export Note</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{route('view.note',$note->id)}}" target="_blank" class="dropdown-item" x-on:click="bShow = !bShow">
                            <i class="fas fa-external-link-alt dropdown-item-icon"></i>
                            <span class="ml-2">View Note</span>
                        </a>
                    </li>
